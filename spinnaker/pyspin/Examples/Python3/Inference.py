...
CHOSEN_INFERENCE_NETWORK_TYPE = InferecneNetworkType.DETECTION
...
NETWORK_FILE_PATH = ("Network_Classification" if ((CHOSEN_INFERENCE_NETWORK_TYPE) \
                                            == InferecneNetworkType.CLASSIFICATION) \
                                            else "Network_Detection")

# The example provides two raw images that can be injected into the camera
# to demonstrate camera inference classification and detection capabilities
# Note: Make sure these files exist on the system and are accessible by the example
...
INJECTED_IMAGE_WIDTH = 640 if CHOSEN_INFERENCE_NETWORK_TYPE == InferecneNetworkType.CLASSIFICATION else 720
INJECTED_IMAGE_HEIGHT = 400 if CHOSEN_INFERENCE_NETWORK_TYPE == InferecneNetworkType.CLASSIFICATION else 540
...
def display_chunk_data(image):
    ...
    try:
        ...
        inference_frame_ID = chunk_data.GetFrameID()
